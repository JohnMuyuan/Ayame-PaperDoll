...
public class VisualConfigEditorScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // 十字的水平线，- 1能居中点，大概
        graphics.hLine(0, width, (height / 2) - 1, LINE_COLOR);
        // 十字的垂直线
        graphics.vLine((width / 2), -1, height, LINE_COLOR);

        // 底下的线
        graphics.hLine(0, width, (height - BORDER_MARGIN), LINE_COLOR);
        // 顶上的线
        graphics.hLine(0, width, BORDER_MARGIN, LINE_COLOR);
        // 左边的线
        graphics.vLine((width - BORDER_MARGIN), -1, height, LINE_COLOR);
        // 右边的线
        graphics.vLine(BORDER_MARGIN, -1, height, LINE_COLOR);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        if (delta != 0) {
            final double newSize = CONFIGS.size.getValue() + (delta / 80);
